...
function canonicalActivityFamilyKey(value, animal) {
  ...
  const speciesAliases = {
    cat: {
      "obedience": "feline obedience",
      "cat obedience": "feline obedience",
      "feline obedience": "feline obedience",
      "rally": "feline rally",
      "cat rally": "feline rally",
      "feline rally": "feline rally",
      "agility": "feline agility",
      "cat agility": "feline agility",
      "feline agility": "feline agility",
      "retrieving": "feline retrieving",
      "cat retrieving": "feline retrieving",
      "feline retrieving": "feline retrieving",
    ...
    dog: {
      "obedience": "canine obedience",
      "dog obedience": "canine obedience",
      "canine obedience": "canine obedience",
      "rally": "canine rally",
      "dog rally": "canine rally",
      "canine rally": "canine rally",
      "agility": "canine agility",
      "dog agility": "canine agility",
      "canine agility": "canine agility",
      "retrieving": "canine retrieving",
      "dog retrieving": "canine retrieving",
      "canine retrieving": "canine retrieving",
      "treibball": "canine treibball",
      "dog treibball": "canine treibball",
      "canine treibball": "canine treibball",
      "trick": "canine trick",
      "dog trick": "canine trick",
      "canine trick": "canine trick"
    },
    horse: {
      "trick": "equine trick",
      "horse trick": "equine trick",
      "equine trick": "equine trick"
    }
  };

  return speciesAliases[species]?.[key] || key;
}
...
function isVersatilityTitleCode(code, species = "") {
  const key = titleCodeKey(code);
  const s = normalizeKey(species);

  const families = {
    dog: ["VND","VAD","VED","VBD","VSD","VGD"],
    cat: ["VNC","VAC","VEC","VBC","VSC","VGC"],
    horse: ["VNH","VAH","VEH","VBH","VSH","VGH"]
  };

  const codes = families[s] || Object.values(families).flat();
  return codes.some(item => titleCodeKey(item) === key);
}

function dedupeTestingCertificateRecords(records) {
  const seen = new Set();
  const output = [];
  ...
  (records || []).forEach(record => {
    const key = [
      ...
      normalizeKey(record?.show_name),
      normalizeKey(record?.class),
      normalizeKey(record?.activity_key),
      String(record?.score ?? "").trim(),
      String(record?.max_score ?? "").trim(),
      String(recordPassed(record)),
      normalizeKey(record?.placement)
    ].join("|");
...
function calculateTitleData(records, animal, titleRules, activityRules, activityTypes, totalRules, herdingRules) {
  ...
  */
  ...
  /*
    Build the non-versatility title stack FIRST.

    Important: remove any old/manual versatility codes before calculation.
    The calculated versatility title is the only V* versatility code allowed
    into the registered name. This prevents stale titles such as VBC remaining
    when the live category calculation says VAC.
  */
  const prefixTitlesBase = uniqueTitleList([
...
function renderVersatilityPanel(animal, titleData) {
  const earnedCodes = titleData?.versatilitySourceCodes || [
    ...(titleData?.prefixTitles || []),
    ...(titleData?.suffixTitles || [])
  ].filter(code => !isVersatilityTitleCode(code, animal?.species));

  const current =
    titleData?.versatilityTitle ||
    calculateVersatilityTitle(animal, earnedCodes);
...
function renderTestingCertificatesPanel(records, animal) {
  const testingRecords = dedupeTestingCertificateRecords(
    (records || []).filter(isTestingCertificateRecord)
  );
  if (!testingRecords.length) return "";

  /*
    Use the deduplicated testing rows for the title cards too, so duplicated
    database rows cannot inflate pass counts or repeat the same test visually.
  */
  const nonTestingRecords = (records || []).filter(record => !isTestingCertificateRecord(record));
  const testingTitleRecords = [...nonTestingRecords, ...testingRecords];
  const titleData = calculateTestingTitles(testingTitleRecords, animal);
